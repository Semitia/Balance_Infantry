...
//	if (Temp.ActualValue > 43.5f) //大于43.5度则按最大计算
//	{
//		Temp.SetPoint = roundf(Temp.ActualValue);
//	}
//	Temp.SetPoint = Temp.SetPoint > 47.5f ? 48.0f : Temp.SetPoint;

	while (fabs(Temp.SetPoint - Temp.ActualValue) > 0.5f)
	{
		TempControl();
		HAL_Delay(1);
	}
